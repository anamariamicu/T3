...
IMDb::IMDb()
    : popular_movies(N, &compare_popularity),
      recent_movies(N, &compare_timestamp) {
...
void IMDb::add_movie(std::string movie_name, std::string movie_id,
                     int timestamp, std::vector<std::string> categories,
  ...
  for (auto it = actor_ids.begin(); it != actor_ids.end(); ++it) {
    ...
    contor++;
    for (auto it2 = actor_ids.begin() + contor;
         it2 != actor_ids.end() && *it2 != *it; ++it2) {
	  // Daca celalalt actor nu exista in graf, se adauga
      if (colleagues.hasNode(*it2) == false) {
        colleagues.addNode(*it2);
      }
      int index_it = colleagues.getIndex(*it),
          index_it2 = colleagues.getIndex(*it2);
...
void IMDb::add_director(std::string name, unsigned int number_actors) {
  ...
  if (most_influential_director == nullptr ||
      director_searched->second.number_actors >
          most_influential_director->number_actors ||
      (director_searched->second.number_actors ==
           most_influential_director->number_actors &&
...
std::string IMDb::get_2nd_degree_colleagues(std::string actor_id) {
  ...
  for (auto it = first_colleagues->begin(); it != first_colleagues->end();
       ++it) {
    std::list<struct data<std::string, int>> *second_colleagues;
    second_colleagues = colleagues.getNeighbors(it->key);
    for (auto it2 = second_colleagues->begin(); it2 != second_colleagues->end();
         ++it2) {
	  // Nu trebuie sa fie colegi de gradul intai!
      if (actor_id != it2->key &&
          colleagues.hasEdge(actor_id, it2->key) == false) {
        auto found = std::find(std::begin(all_second_colleagues),
                               std::end(all_second_colleagues), it2->key);
  ...
  for (auto it = all_second_colleagues.begin();
       it != all_second_colleagues.end(); ++it) {
    result = result + *it + " ";
  }
  if (result == "") {
    return "none";
...
int compare_partners(struct data<std::string, int> *X,
                     struct data<std::string, int> *Y) {
  if (X->value > Y->value) {
    return 1;
