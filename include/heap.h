...
template <typename T> Heap<T>::Heap(int capVect, int (*compare)(T, T)) {
...
template <typename T> void Heap<T>::deleteNodes() {
  dimVect = 0;
  values.resize(1);
  capVect = 1;
}
...
template <typename T> int Heap<T>::parent(int poz) { return (poz - 1) / 2; }

// Returneaza pozitia in vector a copilului stang
template <typename T> int Heap<T>::leftSubtree(int poz) { return 2 * poz + 1; }

// Returneaza pozitia in vector a copilului drept
template <typename T> int Heap<T>::rightSubtree(int poz) { return 2 * poz + 2; }

// Realizeaza operatia de urcare necesara inserarii in Heap
template <typename T> void Heap<T>::pushUp(int poz) {
...
template <typename T> void Heap<T>::pushDown(int poz) {
  while (1) {
...
template <typename T> void Heap<T>::insert(T x) {
...
template <typename T> T Heap<T>::peek() {
  if (dimVect > 0) {
    return values[0];
  }
  return T();
...
template <typename T> T Heap<T>::extractMax() {
  if (dimVect == 0) {
    return T();
  }
  T aux = values[0];
...
template <typename T> bool Heap<T>::hasNodes() {
  if (dimVect == 0) {
    return false;
  }
  return true;
